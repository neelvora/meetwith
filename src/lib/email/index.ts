...
/**
 * Send reminder email to the attendee before their meeting
 */
export async function sendReminderEmail(details: ReminderDetails): Promise<boolean> {
  if (!resend) {
    console.log('Email skipped (Resend not configured):', details.attendeeEmail)
    return false
  }

  const timeUntilMeeting = Math.round((details.startTime.getTime() - Date.now()) / (1000 * 60 * 60))
  const reminderText = timeUntilMeeting <= 1 ? 'starting soon' : `in ${timeUntilMeeting} hours`

  try {
    const { error } = await resend.emails.send({
      from: FROM_EMAIL,
      to: details.attendeeEmail,
      subject: `Reminder: ${details.eventName} with ${details.hostName} ${reminderText}`,
      html: `
        <!DOCTYPE html>
        <html>
          <head>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
              <tr>
                <td>
                  ...
                  <!-- Reminder Badge -->
                  <div style="text-align: center; margin-bottom: 24px;">
                    <div style="display: inline-block; padding: 8px 16px; background: linear-gradient(135deg, rgba(251, 191, 36, 0.2), rgba(251, 191, 36, 0.1)); border: 1px solid rgba(251, 191, 36, 0.3); border-radius: 9999px;">
                      <span style="color: #fbbf24; font-size: 14px; font-weight: 600;">⏰ Meeting Reminder</span>
                    </div>
                  </div>
                  ...
                  <div style="background: linear-gradient(135deg, rgba(255, 255, 255, 0.05), rgba(255, 255, 255, 0.02)); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 16px; padding: 32px;">
                    ...
                    <p style="margin: 0 0 24px 0; color: #fbbf24; font-size: 16px; text-align: center; font-weight: 600;">
                      Starting ${reminderText}!
                    </p>
